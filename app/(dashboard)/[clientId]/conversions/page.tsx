  ...
  return (
    <div>
      ...
      {platform === "ALL" ? (
        ...
        <ConversionsData clientId={clientId} start={start} end={end} platform={platform} locale={locale} />
      )}
    </div>
  );
}
...
async function ConversionsData({
  ...
  const [{ totals, funnel, series }, byCampaign, byDevice] = await Promise.all([
    getConversionsOverview({ clientId, start, end, platform }),
    getConversionsByCampaign({ clientId, start, end, platform }),
